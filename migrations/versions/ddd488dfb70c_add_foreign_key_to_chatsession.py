...
def upgrade():
    ...
    with op.batch_alter_table('chat_message', schema=None) as batch_op:
        batch_op.alter_column('chat_id',
               ...
               nullable=False)
        batch_op.create_foreign_key('fk_chat_message_chat_session', 'chat_session', ['chat_id'], ['id'])

    with op.batch_alter_table('chat_session', schema=None) as batch_op:
        batch_op.alter_column('chat_id',
               existing_type=sa.INTEGER(),
               nullable=False)
        batch_op.create_unique_constraint('uq_chat_session_chat_id', ['chat_id'])
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('chat_session', schema=None) as batch_op:
        batch_op.drop_constraint('uq_chat_session_chat_id', type_='unique')
        batch_op.alter_column('chat_id',
               existing_type=sa.INTEGER(),
               nullable=True)

    with op.batch_alter_table('chat_message', schema=None) as batch_op:
        batch_op.drop_constraint('fk_chat_message_chat_session', type_='foreignkey')
        batch_op.alter_column('chat_id',
               existing_type=sa.INTEGER(),
               nullable=True)
